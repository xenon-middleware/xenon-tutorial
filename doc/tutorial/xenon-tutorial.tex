...
\begin{lstlisting}[style=basic,style=bash,escapeinside={(*@}{@*)}]
time   : 11:50:07.428 (+216 ms)
thread : main
level  : DEBUG
class  : nl.esciencecenter.xenon.examples.files.DirectoryListing:56
message: Reading the URI...
\end{lstlisting}

and:
\begin{lstlisting}[style=basic,style=bash,escapeinside={(*@}{@*)}]
time   : 11:50:07.562 (+350 ms)
thread : main
level  : INFO
class  : nl.esciencecenter.xenon.engine.XenonEngine:166
message: Xenon engine initialized with adaptors: [Adaptor [name=local], ...
\end{lstlisting}
...
To keep things simple, we'll first set up an SSH connection to \url{localhost}, thus effectively connecting ourselves with ourselves. Later on in this document we'll also look at using SSH to connect to two remote systems (see Section~\ref{sec:live-systems}:~\textit{\nameref{sec:live-systems}}). Additionally, we'll set up Docker to run virtual remote systems on your local machine. That way, you can emulate the behavior of one or more remote machines (see Section~\ref{sec:docker}:~\textit{\nameref{sec:docker}}) without the need for these machines to be live all the time. Anyway, that's all for later---let's get started on setting up SSH to \url{localhost} first.
...
\begin{enumerate}
\item{\textit{now adapt DirectoryListing's command line parameters to \url{ssh://jspaaks@lisa.surfsara.nl:22/home/jspaaks} or a simplification thereof}}  %FIXME
\item{\textit{try to run. should not work on account of you not supplying a password. explain that you could set up passwordless login for lisa like we did for localhost, but that instead we will add a pasword in the code and use that}
\item{\textit{now copy DirectoryListing to DirectoryListingWithPwd}}  %FIXME
\item{\textit{add passwordcredential with your password. I know I'm stating the obvious, but make sure you don't accidentally commit this stuff to a public repository}} % FIXME
\item{\textit{try to run DirectoryListing while supplying a password, should work now.}} % FIXME
\item{\textit{analyze the debug messages to show that knonw-hosts is actually used when connecting. then tell them   about the adaptor properties such as autoAddBlah and loadKnownHosts and such, and where to find them $\rightarrow$ javadoc online or generated}}
\item{\textit{Maybe use property to load password from a config file; which you then also should not commit}}
\end{enumerate}
