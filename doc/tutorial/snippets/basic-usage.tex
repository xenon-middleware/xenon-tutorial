...
We want Java to find the fat jar 'Xenon-examples-all.jar' from 'build/libs'. Using paths relative to \mytilde\url{/Xenon}, our classpath thus becomes \url{build/libs/Xenon-examples-all.jar}. However, if we now try to run
\begin{lstlisting}[style=basic,style=bash,escapeinside={(*@}{@*)}]
cd ${HOME}/Xenon
