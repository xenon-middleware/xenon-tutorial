...
For now, we'll just use SSH to connect to our own machine, but later on in this document we'll also look at using SSH to connect to physically remote systems, such as the Lisa\footnote{\url{https://userinfo.surfsara.nl/systems/lisa}} and DAS-4\footnote{\url{http://www.cs.vu.nl/das4/}} systems (see Section~\ref{sec:live-systems}:~\textit{\nameref{sec:live-systems}}). Additionally, we'll set up Docker to run virtualized systems on your local machine, with which you can emulate the behavior of one or more remote machines (see Section~\ref{sec:docker}:~\textit{\nameref{sec:docker}}). Anyway, that's all for later---first we'll make a small digression into setting up the Integrated Development Environment\index{integrated development environment} or IDE\index{IDE} called Eclipse.
